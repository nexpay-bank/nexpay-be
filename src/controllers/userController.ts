...
export const registerUser = async (request: Request, h: ResponseToolkit) => {
  ...
  const [nasabahRole] = await db.select().from(roles).where(eq(roles.role, 'nasabah')).limit(1);
  if (!nasabahRole) {
  ...
  return h.response({ message: 'Registrasi berhasil', user: newUser }).code(201);
};
...
export const login = async (request: Request, h: ResponseToolkit) => {
  ...
  if (!user || !user.isActive) {
    return h.response({ error: 'Username tidak ditemukan atau akun nonaktif' }).code(401);
  }
...
export const logout = async (_request: Request, h: ResponseToolkit) => {
  ...
  return h.response({ message: 'Logout berhasil (token dihapus client)' }).code(200);
};

export const updateProfilePhoto = async (request: Request, h: ResponseToolkit) => {
...
export const deleteOwnAccount = async (request: Request, h: ResponseToolkit) => {
  ...
  if (!deactivated) {
    return h.response({ message: 'Akun tidak ditemukan atau sudah tidak aktif.' }).code(404);
  }
